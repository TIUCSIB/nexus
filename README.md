...
# 后台运行
Start-Process -FilePath "./bin/nexus.exe" -WindowStyle Hidden
`
...
`ash
./nexus-agent --panel https://your-panel.com --token YOUR_REGISTER_TOKEN --name my-node
`
...
export NEXUS_TOKEN=YOUR_REGISTER_TOKEN
...
`
1. 面板创建节点 -> 复制 register_token
2. 服务器运行: ./nexus-agent --panel URL --token TOKEN --name 节点名
